...
def find_metric_records(payload: Any, value_keys: tuple[str, ...]) -> tuple[list[dict], str]:
    """Recursively find the largest list of dict records containing a metric key.

    Returns (records, path). This tolerates wrappers such as data.data or result.data
    without assuming a single response envelope.
    """
    best: list[dict] = []
    best_path = ""
...
def safe_shape_summary(payload: Any) -> str:
    """Return structure-only diagnostics; never response values."""
