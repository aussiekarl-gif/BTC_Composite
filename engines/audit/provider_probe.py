...
def _classify_coinglass(provider, secret_name, token, call: Callable[[str], requests.Response]):
    ...
    mx=r.headers.get("API-KEY-MAX-LIMIT", ""); used=r.headers.get("API-KEY-USE-LIMIT", "")
    common=dict(http_status=r.status_code, rate_limit_max=mx, rate_limit_used=used)
    if r.status_code == 429:
        return ProbeResult(provider, secret_name, True, "RATE LIMITED", detail="No retry performed", **common)
    if r.status_code != 200:
        if r.status_code in (401,403):
            return ProbeResult(provider, secret_name, True, "AUTH / PLAN BLOCKED", detail="HTTP access blocked", **common)
        return ProbeResult(provider, secret_name, True, "HTTP ERROR", detail=f"HTTP {r.status_code}", **common)
    try:
        payload = r.json()
    except ValueError:
        return ProbeResult(provider, secret_name, True, "INVALID JSON", detail="HTTP 200 but response was not JSON", **common)
    if isinstance(payload, dict) and "code" in payload:
        api_code = payload.get("code")
        msg = _safe_text(payload.get("msg"))
        success_codes = {0, "0", 200, "200"}
        if api_code not in success_codes:
            return ProbeResult(provider, secret_name, True, "API / PLAN BLOCKED", detail=f"CoinGlass code {api_code}: {msg}" if msg else f"CoinGlass code {api_code}", **common)
        if "data" not in payload:
            return ProbeResult(provider, secret_name, True, "API RESPONSE EMPTY", detail=f"CoinGlass code {api_code}: {msg}" if msg else f"CoinGlass code {api_code}; no data field", **common)
...
def run_provider_probes(st) -> list[dict]:
    ...
    out.append(_classify_coinglass("CoinGlass — Puell Multiple",cgn,cg,lambda key: requests.get("https://open-api-v4.coinglass.com/api/index/puell-multiple",headers={"CG-API-KEY":key,"Accept":"application/json"},timeout=15)))
