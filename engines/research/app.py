...
def add_risk_indicators(data, risk_model, params):
    ...
    if risk_model == "Research WF Power Law":
        result["risk_score"] = result["power_law_score"].clip(0, 1)
        result["raw_risk_score"] = result["power_law_score"].clip(0, 1)
        result["risk_components_available"] = result["power_law_score"].notna().astype(int)
...
st.title("Bitcoin Dynamic DCA V5.9 RESEARCH — Simplified Smart DCA")
...
if mode == "DCA Backtest":
    ...
    with st.spinner("Loading historical BTC and AUD/USD data..."):
        df_full = fetch_btc_history(params["start_date"] - timedelta(days=730), params["end_date"])
