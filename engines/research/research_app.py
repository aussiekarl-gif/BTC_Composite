...
def _multiplier_curve_controls(default_curve, storage_key):
    ...
    with st.expander("Advanced Multiplier Settings", expanded=False):
        st.caption(
            "Custom settings affect V5.9 only. Multipliers must stay level or decrease as risk rises."
        )
...
with st.sidebar:
    ...
    active_smart_dca_curve, custom_curve_active = _multiplier_curve_controls(
        SMART_DCA_POINTS, "research_multiplier_curve"
    )
    ...
    if mode == "DCA Backtest":
        ...
        st.caption(
            "Custom V5.9 multiplier curve is active."
            if custom_curve_active else
            "V5.9 default: 2.00× at risk 0.00, 1.00× at risk 0.50, and 0.80× at risk 1.00."
        )
