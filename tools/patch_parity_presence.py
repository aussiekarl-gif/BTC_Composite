...
old = '''def add_or_gap(label, current_series, central_col, current_source):
...
'''
new = '''def add_or_gap(label, current_series, central_col, current_source):
...
'''
if old not in s:
    raise SystemExit('add_or_gap block not found')
s = s.replace(old, new, 1)
...
'''
new = '''if "Central observations" not in parity.columns:
